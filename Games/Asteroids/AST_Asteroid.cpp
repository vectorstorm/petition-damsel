...
physSprite( list, ColFlag_Enemy, ColFlag_Enemy | ColFlag_Shot | ColFlag_Player ),
m_type(type),
m_spawned(false)
{
}
...
astAsteroid::Spawn(const vsVector2D &where_in, const vsVector2D &baseVelocity, float radius_in, bool noOverlap)
{
	...
	while ( !m_spawned )
	{
			...
			!core::GetGame()->GetCollision()->CollisionTest( where, nextPos, radius, m_colObject->GetTestFlags() ) )
		{
			...
			float motionX = vsRandom::GetFloat( -50.0f, 50.0f );	
			float motionY = vsRandom::GetFloat( -50.0f, 50.0f );
...
astAsteroid::CollisionCallback( const colEvent &event )
{
	if ( event.colFlags & ColFlag_Shot )
	{
		...
		astGame::Instance()->GetShrapnelEmitter()->SpawnImmediateBurst(particles[m_type], GetPosition(), 100.0f, 20.0f);

		
		if ( nextType < Type_MAX )
		{
			astAsteroidPool::Instance()->SpawnAsteroid( nextType, GetPosition(), GetVelocity(), 
													   m_colObject->GetBoundingCircle().radius * 1.0f, false );
			astAsteroidPool::Instance()->SpawnAsteroid( nextType, GetPosition(), GetVelocity(), 
													   m_colObject->GetBoundingCircle().radius * 1.0f, false );
		}
...
m_respawning(false)
{
	m_largeList = vsDisplayList::Load("Asteroid.vec");
	m_mediumList = vsDisplayList::Load("MediumAsteroid.vec");
	m_smallList = vsDisplayList::Load("SmallAsteroid.vec");
...
void
astAsteroidPool::WrapAround( vsCamera *camera )
{
	vsVector2D v;
	float r;
