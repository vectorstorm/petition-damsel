...
astGame::Init()
{
	...
	// Init emitters
	m_sparkEmitter = new physEmitter("Thrust.vec", 0.1f, 160);
	m_shrapnelEmitter = new physEmitter("Shrapnel.vec", 0.1f, 160);
	...
	for ( int i = 0; i < m_starCount; i++ )
	{
		...
		m_star[i]->RegisterOnLayer(0);
	}
	
	m_howToPlayScreen = new astHowToPlay;
	m_howToPlayScreen->RegisterOnLayer(1);
