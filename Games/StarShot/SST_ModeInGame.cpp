...
m_game(game)
{
	...
	// Init Screen
	vsScreen::Instance()->GetLayer(1)->RegisterEntityOnTop( m_hud );
	...
	m_rocketThrust = new sndSample("RocketThrust.wav");
	m_rocketPop = new sndSample("RocketPop.wav");
	m_crowdMurmur = new sndSample("SmallAudienceChatter.wav");
	...
	//m_emitter = new physEmitter("Boom.vec", 0.1f, 800);
	m_emitter = new sstFireworks(5000);
	vsScreen::Instance()->GetLayer(0)->RegisterEntityOnTop( m_emitter );
	...
	m_music = new sndMusic("BigBoom.mp3", false);
}
...
{
	fsFile *saveFile = new fsFile(filename, fsFile::MODE_Write);
	fsRecord r;
...
{
	fsFile *saveFile = new fsFile(filename, fsFile::MODE_Read);
	fsRecord r;
	...
	if ( !saveFile->AtEnd() )
	{
		...
		vsAssert( r.GetLabel() == vsString("BeatCount"), "Didn't find expected BeatCount record!" );
		vsAssert( m_beatArraySize >= r.GetArgAsInt(0), "Too many beats specified in data file!" );
		
		while ( saveFile->Record(&r) )
		{
			vsAssert( r.GetLabel() == vsString("BeatTime"), "Found unexpected label!" );
			
			m_beat[ m_beatCount++ ] = r.GetArgAsFloat(0);
		}
	}
...
sstModeInGame::Update(float timeStep)
{
	...
	sysInput *input = m_game->GetInput();
