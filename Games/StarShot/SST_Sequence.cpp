...
sstSequence::ClampBeats()
{
	...
	float fov = vsScreen::Instance()->GetLayer(0)->GetFOV();
	float screenHeight = fov;
	float screenWidth = screenHeight * vsScreen::Instance()->GetAspectRatio();
...
//	bool clampedSuccessfully = false;
	...
	for ( int i = 0; i < m_beatCount; i++ )
	{
		vsTransform t = GetTransformAtTime( m_segment[i].m_time );
...
sstSequence::Update( float timeStep )
{
	...
	while ( m_nextBeat < m_beatCount && musicTime + c_rocketTime > m_beat[m_nextBeat] )
	{
		...
		if ( r )
		{
			...
			vsTransform transformAtImpact = GetTransformAtTime( m_beat[m_nextBeat] );
...
void
sstSequence::Draw( vsDisplayList *list )
{
	float musicTime = m_mode->GetMusicTime();
...
sstSequence::Spawn()
{
	vsScreen::Instance()->GetLayer(0)->RegisterEntityOnTop( this );
	m_spawned = true;
}
